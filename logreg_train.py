...
def train_logistic_regression_gradient_descent(train_X: pd.DataFrame, train_y: pd.DataFrame, learning_rate: float, iterations: int, house: str):
    ...
    for i in range(iterations):
        ...
        # Calculate the prediction error
        # train_X @ theta = z=θ0​(1)+θ1​(x1​)+θ2​(x2​)+...+θn​(xn​)
        # It multiplies each feature by its corresponding weight and sums them up
        # pass it through the sigmoid function to squash it into a probability
        prediction = sigmoid(train_X @ theta) #0-1 probality
        error = prediction - train_y

        # Calculate the gradient (partial derivative)
        # use that error (multiplied by X.T) to nudge the weights (theta) in the right direction
        # X.T stands for the transpose of the matrix X
        # (m×n), then X.T will have a shape of (n×m)
        # 1/m * ​(X.T * (error)) = 1/m * (all features of row i) * error
        # Gradient tells each weight how to adjust.
        gradient = (1/m) * (train_X.T @ error)

        # Update weights for next iteraction training
        """
        Every iteration:
            Compute predictions
            Compute error
